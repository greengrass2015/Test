...
public class ArticleAssemble {
	...
	public Article articleAssemble( ){
		...
		Section sectionTemp = new Section(subTitle, paragraphList); //标题为空
		...
		paragraphList.addAll(cpiComments.indicatorComments(cpiCommentsFile, analysisOfStats));
		...
		// 再添加专家引用
		EconomicComments ppiComments = new EconomicComments();
		paragraphList.addAll(ppiComments.indicatorComments(ppiCommentsFile, analysisOfStats));
